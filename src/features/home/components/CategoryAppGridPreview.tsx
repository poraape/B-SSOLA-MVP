...
export const CategoryAppGridPreview: React.FC<CategoryAppGridPreviewProps> = ({
  ...
  return (
    <section className="space-y-6 rounded-[20px] border border-slate-200/70 bg-white/45 p-4 shadow-[0_16px_36px_-26px_rgba(15,23,42,0.35)] backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-900/35 sm:space-y-8 sm:p-5 md:p-6">
      ...
      <div className="grid grid-cols-1 gap-3 min-[360px]:grid-cols-2 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
        {categories.map((category) => (
          ...
          >
            ...
            <div className="relative z-10 flex h-full min-h-[9rem] flex-col items-center justify-between px-3 pb-3.5 pt-3.5 text-center transition-transform duration-200 group-hover:-translate-y-1 group-active:translate-y-0.5 sm:min-h-[10.5rem] sm:pb-4 sm:pt-5">
              <div className="relative inline-flex transition-transform duration-200 group-hover:scale-[1.06] group-active:scale-95">
                {getPremiumCategoryIconByName(
                  getCategoryIcon(category.id),
                  'h-[4.5rem] w-[4.5rem] drop-shadow-[0_10px_18px_rgba(15,23,42,0.28)] sm:h-22 sm:w-22',
                )}
      ...
      <div className="flex justify-center">
        <button
          ...
          className="min-h-[44px] rounded-[20px] border border-slate-300/80 bg-white/75 px-6 py-3 text-sm font-bold text-slate-700 shadow-[0_10px_22px_-18px_rgba(15,23,42,0.4)] backdrop-blur-md transition-all hover:scale-[1.03] hover:border-blue-400 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:border-blue-500 dark:hover:text-blue-300"
        >
