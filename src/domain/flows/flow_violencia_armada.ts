...
export const flow_violencia_armada: FlowSpec = {
  meta: {
    ...
    severity: 'CRITICAL',
    keywords: ['violencia armada', 'tiroteio', 'abrigo seguro', 'protecao imediata'],
    status: 'EXISTING',
  },
  risk: {
    ...
    escalationRules: [
      {
        ...
        ifAny: ['arma_visivel', 'disparos_ouvidos'],
        then: {
          riskLevel: 'CRITICAL',
          outcome: 'outcome_lockdown_critico',
          flags: ['call_190', 'notify_management', 'document_formal', 'do_not_leave_alone'],
        },
        rationale: 'Risco imediato a vida exige resposta de seguranca sem atraso.',
      },
      {
        id: 'rule_ameaca_na_regiao',
        ifAny: ['suspeita_ameaca_externa'],
        then: {
          riskLevel: 'HIGH',
    ...
    protectiveFactors: ['plano_de_emergencia_ativo', 'equipe_treinada', 'salas_com_trava_funcional'],
    riskSignals: [
      {
        id: 'arma_visivel',
        label: 'Pessoa com arma visivel na escola ou entorno imediato',
        examples: ['arma na entrada', 'agressor armado no patio'],
        weight: 3,
      },
      {
        id: 'disparos_ouvidos',
        label: 'Som de disparos ou explosoes proximas',
        examples: ['barulho de tiros', 'relato simultaneo de disparos'],
        weight: 3,
      },
      {
        id: 'suspeita_ameaca_externa',
        label: 'Ameaca recebida por mensagem ou relato confiavel',
        examples: ['aviso de ataque em rede social', 'alerta da comunidade'],
        weight: 2,
      },
    ],
    recommendedActionsByRisk: {
      MODERATE: ['Manter observacao com a gestao', 'Registrar informacoes iniciais'],
      HIGH: ['Avisar gestao escolar', 'Restringir circulacao em areas expostas', 'Entrar em contato com responsaveis'],
      CRITICAL: ['Acionar 190 imediatamente', 'Ativar abrigo seguro/lockdown', 'Nao deixar estudantes desacompanhados'],
    },
    recommendedServiceTagsByRisk: {
      MODERATE: ['ASSISTENCIA_SOCIAL_ESCOLAR'],
      HIGH: ['ASSISTENCIA_SOCIAL_ESCOLAR', 'DELEGACIA'],
  ...
  steps: [
    {
      ...
      riskSignals: ['arma_visivel', 'disparos_ouvidos'],
    },
    {
      id: 'step_2',
      type: 'action',
    ...
    },
    {
      ...
      actions: [
        { label: 'Sim', next: 'outcome_lockdown_critico' },
        { label: 'Nao, e uma ameaca sem ataque em curso', next: 'outcome_protecao_preventiva' },
      ],
      riskSignals: ['arma_visivel', 'disparos_ouvidos', 'suspeita_ameaca_externa'],
    },
    {
      id: 'step_4',
      type: 'question',
      ...
      actions: [
        { label: 'Nao', next: 'outcome_lockdown_critico' },
        { label: 'Sim', next: 'outcome_transicao_segura' },
      ],
    },
  ],
  outcomes: [
