...
export const flow_ameaca_externa: FlowSpec = {
  meta: {
    ...
    severity: 'HIGH',
    keywords: ['ameaca externa', 'entorno escolar', 'protecao coletiva', 'seguranca'],
    status: 'TO_CREATE',
  },
  risk: {
    ...
    escalationRules: [
      {
        ...
        ifAny: ['agressor_nas_proximidades', 'violencia_em_andamento'],
        then: {
          riskLevel: 'CRITICAL',
      ...
      },
      {
        ...
        ifAny: ['alerta_mensagem_confiavel', 'movimentacao_suspeita_recorrente'],
        then: {
          riskLevel: 'HIGH',
    ...
    protectiveFactors: ['controle_de_acesso', 'plano_de_contingencia', 'comunicacao_interna_rapida'],
    riskSignals: [
      {
        id: 'agressor_nas_proximidades',
        label: 'Pessoa agressiva nas proximidades da escola',
        examples: ['ameaca na entrada', 'tentativa de invasao'],
        weight: 3,
      },
      {
        id: 'violencia_em_andamento',
        label: 'Violencia em andamento no entorno imediato',
        examples: ['briga grave na porta', 'barulho de disparos nas proximidades'],
        weight: 3,
      },
      {
        id: 'alerta_mensagem_confiavel',
        label: 'Ameaca recebida por canal confiavel',
        examples: ['comunicacao oficial', 'relato simultaneo de equipe e comunidade'],
        weight: 2,
      },
      {
        id: 'movimentacao_suspeita_recorrente',
        label: 'Movimentacao suspeita recorrente no entorno',
        examples: ['rondas de intimidacao', 'tentativas repetidas de acesso indevido'],
        weight: 2,
      },
    ],
    recommendedActionsByRisk: {
      MODERATE: ['Reforcar observacao e registro com a gestao', 'Revisar controle de acesso da escola'],
      HIGH: ['Avisar gestao escolar', 'Restringir circulacao em areas expostas', 'Informar responsaveis com orientacao objetiva'],
      CRITICAL: ['Acionar 190 imediatamente', 'Ativar protecao interna da escola', 'Nao deixar estudantes sozinhos em deslocamento'],
    },
  ...
  steps: [
    {
      ...
      riskSignals: ['agressor_nas_proximidades', 'violencia_em_andamento'],
    },
    {
      id: 'step_2',
      type: 'action',
      action: 'Feche acessos, mantenha estudantes em area protegida e acione a gestao.',
      riskSignals: ['agressor_nas_proximidades'],
    },
    {
      id: 'step_3',
      type: 'question',
    ...
    },
    {
      ...
      actions: [
        { label: 'Sim', next: 'outcome_contencao_preventiva' },
        { label: 'Nao', next: 'outcome_monitoramento_reforcado' },
      ],
      riskSignals: ['alerta_mensagem_confiavel', 'movimentacao_suspeita_recorrente'],
    },
  ],
  outcomes: [
    {
    ...
    },
    {
      ...
      timeline: 'Horas',
      riskLevel: 'HIGH',
      serviceTags: ['DELEGACIA', 'ASSISTENCIA_SOCIAL_ESCOLAR'],
      flags: ['notify_management', 'notify_guardians', 'document_formal'],
    },
    {
