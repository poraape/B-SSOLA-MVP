...
export const flow_crise_respiratoria: FlowSpec = {
  meta: {
    ...
    severity: 'HIGH',
    keywords: ['crise respiratoria', 'asma', 'falta de ar', 'urgencia escolar'],
    status: 'TO_CREATE',
  },
  risk: {
    ...
    escalationRules: [
      {
        ...
        ifAny: ['falta_ar_intensa', 'sinais_hipoxia'],
        then: {
          riskLevel: 'CRITICAL',
      ...
      },
      {
        ...
        ifAny: ['chiado_persistente', 'resposta_insuficiente_medida_inicial'],
        then: {
          riskLevel: 'HIGH',
    ...
    protectiveFactors: ['ambiente_ventilado', 'adulto_referencia_presente', 'plano_individual_de_saude'],
    riskSignals: [
      {
        id: 'falta_ar_intensa',
        label: 'Falta de ar intensa com dificuldade para falar',
        examples: ['fala entrecortada', 'uso de musculatura do pescoco para respirar'],
        weight: 3,
      },
      {
        id: 'sinais_hipoxia',
        label: 'Sinais de baixa oxigenacao',
        examples: ['labios arroxeados', 'confusao por falta de ar'],
        weight: 3,
      },
      {
        id: 'chiado_persistente',
        label: 'Chiado no peito persistente sem alivio',
        examples: ['chiado continuo por varios minutos'],
        weight: 2,
      },
      {
        id: 'resposta_insuficiente_medida_inicial',
        label: 'Sem melhora apos medidas iniciais seguras',
        examples: ['sem alivio apos repouso e orientacao'],
        weight: 2,
      },
    ],
    recommendedActionsByRisk: {
      MODERATE: ['Acolher em local ventilado e observar', 'Orientar avaliacao na UBS de referencia'],
      HIGH: ['Avisar gestao escolar', 'Acionar responsaveis', 'Encaminhar para UPA/Hospital ou UBS conforme gravidade'],
      CRITICAL: ['Acionar 192 imediatamente', 'Nao deixar a pessoa sozinha', 'Manter posicao confortavel e ambiente ventilado'],
    },
  ...
  steps: [
    {
      ...
      riskSignals: ['falta_ar_intensa', 'sinais_hipoxia'],
    },
    {
      id: 'step_2',
      type: 'action',
      action: 'Mantenha a pessoa sentada, em local ventilado, e acione a gestao escolar.',
      riskSignals: ['chiado_persistente', 'resposta_insuficiente_medida_inicial'],
    },
    {
      id: 'step_3',
      type: 'question',
      ...
      actions: [
        { label: 'Sim', next: 'outcome_urgencia_respiratoria' },
    ...
    },
    {
      ...
      actions: [
        { label: 'Sim', next: 'outcome_encaminhamento_prioritario' },
        { label: 'Nao', next: 'outcome_observacao_respiratoria' },
      ],
      riskSignals: ['chiado_persistente', 'resposta_insuficiente_medida_inicial'],
    },
  ],
