...
export const flow_bullying: FlowSpec = {
  meta: {
    ...
    severity: 'HIGH',
    keywords: ['bullying', 'cyberbullying', 'humilhacao recorrente', 'protecao escolar'],
    status: 'EXISTING',
  },
  risk: {
    ...
    escalationRules: [
      {
        ...
        ifAny: ['ameaca_grave', 'exposicao_viral_humilhante'],
        then: {
          riskLevel: 'CRITICAL',
          outcome: 'outcome_protecao_imediata',
          flags: ['notify_management', 'notify_guardians', 'document_formal', 'do_not_leave_alone'],
        },
        rationale: 'Ameacas graves e exposicao humilhante de alto alcance podem gerar risco imediato.',
      },
      {
        id: 'rule_repeticao_sistematica',
        ifAny: ['repeticao_agressoes', 'impacto_emocional_importante'],
        then: {
          riskLevel: 'HIGH',
    ...
    protectiveFactors: ['adulto_referencia_disponivel', 'canal_de_escuta_ativa', 'mediacao_restaurativa'],
    riskSignals: [
      {
        id: 'ameaca_grave',
        label: 'Ameaca grave de agressao fisica ou exposicao violenta',
        examples: ['ameaca de bater apos a aula', 'chantagem com imagens intimas'],
        weight: 3,
      },
      {
        id: 'exposicao_viral_humilhante',
        label: 'Conteudo humilhante com ampla circulacao digital',
        examples: ['video de humilhacao em grupos', 'perfil falso para ataques'],
        weight: 3,
      },
      {
        id: 'repeticao_agressoes',
        label: 'Agressoes verbais/sociais repetidas contra o mesmo estudante',
        examples: ['apelidos ofensivos diarios', 'isolamento forcado no recreio'],
        weight: 2,
      },
      {
        id: 'impacto_emocional_importante',
        label: 'Sofrimento emocional relevante associado ao bullying',
        examples: ['medo de vir a escola', 'queda acentuada de participacao'],
        weight: 2,
      },
    ],
    recommendedActionsByRisk: {
      MODERATE: ['Escuta qualificada e registro pedagogico', 'Acordos de convivencia e monitoramento'],
      HIGH: ['Avisar gestao escolar', 'Registrar formalmente', 'Acionar responsaveis e plano de intervencao'],
      CRITICAL: ['Garantir protecao imediata da vitima', 'Nao deixar estudante sozinho em risco grave', 'Ativar protocolo interno imediato'],
    },
    recommendedServiceTagsByRisk: {
      MODERATE: ['MEDIACAO_RESTAURATIVA', 'ASSISTENCIA_SOCIAL_ESCOLAR'],
      HIGH: ['MEDIACAO_RESTAURATIVA', 'ASSISTENCIA_SOCIAL_ESCOLAR', 'CAPS_IJ'],
      CRITICAL: ['CONSELHO_TUTELAR', 'CAPS_IJ', 'DELEGACIA'],
    },
  },
  steps: [
    {
      ...
      riskSignals: ['repeticao_agressoes', 'impacto_emocional_importante'],
    },
    {
      id: 'step_2',
      type: 'question',
      ...
      actions: [
        ...
        { label: 'Nao', next: 'step_3' },
      ],
      riskSignals: ['ameaca_grave', 'exposicao_viral_humilhante'],
    },
    {
      id: 'step_3',
      type: 'question',
      ...
      actions: [
        { label: 'Sim', next: 'outcome_plano_intervencao' },
        { label: 'Nao, foi episodio isolado', next: 'outcome_mediacao_preventiva' },
      ],
      riskSignals: ['repeticao_agressoes', 'impacto_emocional_importante'],
    },
  ],
  outcomes: [
    {
      ...
      timeline: 'Imediato',
      riskLevel: 'CRITICAL',
      serviceTags: ['CONSELHO_TUTELAR', 'CAPS_IJ', 'DELEGACIA'],
      flags: ['notify_management', 'notify_guardians', 'do_not_leave_alone', 'document_formal', 'contact_council'],
    },
    {
      ...
      timeline: 'Horas',
      riskLevel: 'HIGH',
      serviceTags: ['MEDIACAO_RESTAURATIVA', 'ASSISTENCIA_SOCIAL_ESCOLAR', 'CAPS_IJ'],
      flags: ['notify_management', 'notify_guardians', 'document_formal'],
    },
    {
