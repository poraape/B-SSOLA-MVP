...
export const flow_risco_estrutural: FlowSpec = {
  meta: {
    ...
    severity: 'HIGH',
    keywords: ['risco estrutural', 'rachadura grave', 'isolamento de area', 'evacuacao'],
    status: 'EXISTING',
  },
  risk: {
    ...
    escalationRules: [
      {
        ...
        ifAny: ['sinais_colapso_imediato', 'queda_de_elementos'],
        then: {
          riskLevel: 'CRITICAL',
      ...
      },
      {
        ...
        ifAny: ['rachadura_grave', 'interdicao_parcial_necessaria'],
        then: {
          riskLevel: 'HIGH',
    ...
    protectiveFactors: ['plano_de_contingencia_predial', 'rotas_de_saida_livres', 'equipe_orientada_para_evacuacao'],
    riskSignals: [
      {
        id: 'sinais_colapso_imediato',
        label: 'Estalos fortes, inclinacao ou afundamento visivel',
        examples: ['barulho de rompimento', 'parede cedendo'],
        weight: 3,
      },
      {
        id: 'queda_de_elementos',
        label: 'Queda de parte de teto, reboco ou estrutura',
        examples: ['placas caindo', 'pedaos de concreto no chao'],
        weight: 3,
      },
      {
        id: 'rachadura_grave',
        label: 'Rachaduras amplas com progressao aparente',
        examples: ['trinca aumentando no mesmo dia'],
        weight: 2,
      },
      {
        id: 'interdicao_parcial_necessaria',
        label: 'Area com risco que exige bloqueio imediato',
        examples: ['sala interditada', 'corredor com risco de queda'],
        weight: 2,
      },
    ],
    recommendedActionsByRisk: {
      MODERATE: ['Registrar observacao predial', 'Monitorar area com equipe de apoio'],
      HIGH: ['Avisar gestao escolar', 'Isolar area afetada', 'Entrar em contato com responsaveis'],
      CRITICAL: ['Evacuar imediatamente', 'Acionar 190 e defesa civil local', 'Nao deixar estudantes sozinhos durante retirada'],
    },
    recommendedServiceTagsByRisk: {
      MODERATE: ['ASSISTENCIA_SOCIAL_ESCOLAR'],
      HIGH: ['ASSISTENCIA_SOCIAL_ESCOLAR'],
  ...
  steps: [
    {
      ...
      riskSignals: ['rachadura_grave', 'interdicao_parcial_necessaria'],
    },
    {
      id: 'step_2',
      type: 'action',
      action: 'Isole imediatamente a area de risco e organize deslocamento para local seguro.',
      riskSignals: ['interdicao_parcial_necessaria', 'queda_de_elementos'],
    },
    {
      id: 'step_3',
      type: 'question',
      ...
      actions: [
        { label: 'Sim', next: 'outcome_evacuacao_critica' },
        { label: 'Nao', next: 'step_4' },
      ],
      riskSignals: ['sinais_colapso_imediato', 'queda_de_elementos'],
    },
    {
      id: 'step_4',
      type: 'question',
      ...
      actions: [
        { label: 'Sim', next: 'outcome_isolamento_preventivo' },
        { label: 'Nao', next: 'outcome_monitoramento_predial' },
      ],
      riskSignals: ['rachadura_grave', 'interdicao_parcial_necessaria'],
    },
  ],
